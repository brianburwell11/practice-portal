...
Drop audio files to simulate the lazy decode flow. All files start as "pending" — click "decode" on individual stems to see decode time and PCM memory cost, simulating what happens when a muted stem gets unmuted.
